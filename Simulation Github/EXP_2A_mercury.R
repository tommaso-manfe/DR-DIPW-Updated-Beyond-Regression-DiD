...
#              PROPENSITY SCORES CORRECTLY SPECIFIED, OUTCOME REGRESSION CORRECTLY SPECIFIED
...
res=foreach(i=1:10000, .combine = 'rbind', .packages = c("glmnet", "trust", "randomForest")) %dopar% {
  ...
  ti_nt<- stats::plogis(- z1 + 0.5 * z2 - 0.25 * z3 - 0.1 * z4)
  ti_t <- stats::plogis(+ z1 - 0.5 * z2 + 0.25 * z3 + 0.1 * z4)
  ti=d*ti_t+(1-d)*ti_nt
...
ptime
print(res)
saveRDS(res, "EXP_2A_storage.rds")
